...
SESSIONS = [
    (1, "Running Python, Variables & Types",
     "REPL vs script, the five core types, f-strings, casting, tracebacks — then operators, string methods, import math.", False),
    (2, "The Dynamic-Typing Traps",
     "== vs is, True==1, float precision, truthiness, isinstance — then None, conversions, nan, Decimal.", False),
    (3, "Control Flow: Conditionals & Loops",
     "if/elif/else, chained comparisons, for/while, enumerate/zip — then ternary, match/case, for/else, loop patterns.", False),
    (4, "Data Structures",
     "list/tuple/dict/set, slicing, comprehensions, aliasing — then Counter, defaultdict, set algebra, multi-key sorts.", False),
    (5, "Functions, Scope & Reusability",
     "def, *args/**kwargs, scope, the mutable-default bug — then closures, a first decorator, doctests.", False),
    (6, "Recursion & Recursive Thinking",
     "base + recursive case, the call stack, nested data — then memoization, binary search, trees, the explicit stack.", False),
    (7, "Exceptions & Defensive Code",
     "try/except, raise, EAFP, a first pytest test — then the exception hierarchy, raise from, logging, parametrize.", False),
    (8, "Files, Libraries & Research Data",
     "open/with, CSV as dicts, statistics — then pathlib, encodings, JSON, datetime, seeded random, pandas teaser.", False),
